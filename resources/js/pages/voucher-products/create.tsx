...
export default function Create({ personnel_types, voucher_categories, products }: CreateProps) {
            ...
            <div className="flex h-full flex-1 flex-col gap-6 overflow-x-auto rounded-2xl p-4 md:p-6">
                <div className="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm dark:border-sky-950 dark:bg-black">
                    <div className="flex items-start gap-4">
                        <div className="rounded-xl bg-indigo-100 p-3 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-300"><PackagePlus className="size-6" /></div>
                        <div>
                            <p className="text-sm font-medium text-slate-500 dark:text-slate-300">Voucher management</p>
                            <h1 className="mt-1 text-2xl font-semibold text-slate-900 dark:text-slate-100">Create a voucher</h1>
                            <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Choose its audience, category, and included products.</p>
                        </div>
                    </div>
                </div>

                <Form action="/vouchers" method="post" className="space-y-6">
                    {({ errors, processing }) => (
                        <>
                            <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm dark:border-sky-950 dark:bg-black">
                                ...
                                <div className="grid gap-5 md:grid-cols-2">
                                    <div className="space-y-2">
                                        ...
                                        <select id="personnel_type_id" name="personnel_type_id" className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:outline-none dark:border-slate-800 dark:bg-black dark:text-slate-100" required defaultValue="">
                                            <option value="" disabled>Select personnel type</option>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <select id="category_id" name="category_id" className="flex h-10 w-full rounded-md border border-slate-200 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm transition focus:border-indigo-500 focus:outline-none dark:border-slate-800 dark:bg-black dark:text-slate-100" required defaultValue="">
                                            <option value="" disabled>Select voucher category</option>
                            ...
                            <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm dark:border-sky-950 dark:bg-black">
                                <div className="flex flex-col gap-2 border-b border-slate-200 px-5 py-4 dark:border-sky-950 sm:flex-row sm:items-center sm:justify-between">
                                    <div><h2 className="font-semibold text-slate-900 dark:text-slate-100">Products</h2><p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Select one or more products and set the quantity for each.</p></div>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="min-w-full text-left text-sm">
                                        ...
                                        <tbody className="divide-y divide-slate-200 dark:divide-sky-950">
                                            {products.length === 0 ? <tr><td colSpan={7} className="px-5 py-10 text-center text-slate-500">No products are available.</td></tr> : products.map((product) => {
                            ...
                            <div className="flex justify-end"><Button type="submit" disabled={processing || selectedProducts.length === 0} className="min-w-40 bg-green-600 text-white hover:bg-green-700"><Check className="size-4" />{processing ? 'Creating voucher...' : 'Create voucher'}</Button></div>
                        </>
                    )}
                </Form>
            </div>
...
Create.layout = { breadcrumbs: [{ title: 'Vouchers', href: '/vouchers' }, { title: 'Create voucher', href: '/vouchers/create' }] };
