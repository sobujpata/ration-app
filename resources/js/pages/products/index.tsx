...
export default function Index() {
    ...
    const filteredFatchData = datas.filter((products) => {
    ...
    const handleDelete = async (id: number) => {
        ...
        try {
            ...
            setData((currentFatchData) => currentFatchData.filter((product) => product.id !== id));
        } catch (deleteError) {
                ...
                <div className="rounded-2xl border border-slate-200 bg-white px-6 py-5 shadow-sm dark:bg-black dark:border-sky-950">
                    <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                        <div>
                            ...
                            <h1 className="mt-1 text-2xl font-semibold text-slate-900 dark:text-slate-100">Products Management</h1>
                        </div>
                        ...
                        <table className="min-w-full border-collapse text-left text-sm dark:text-slate-100 text-slate-700">
                            <thead className="dark:bg-black dark:text-slate-100 bg-slate-100 text-slate-700">
                                <tr>
                                    ...
                                    <th className="dark:border-sky-950 border-b border-slate-200 px-4 py-3 font-semibold">Unit</th>
                                    <th className="dark:border-sky-950 border-b border-slate-200 px-4 py-3 font-semibold">Quantity</th>
                                    <th className="dark:border-sky-950 border-b border-slate-200 px-4 py-3 font-semibold">Unit Price</th>
                                    <th className="dark:border-sky-950 border-b border-slate-200 px-4 py-3 font-semibold">Unit Sale Price</th>
                                    <th className="dark:border-sky-950 border-b border-slate-200 px-4 py-3 font-semibold">SKU</th>
                            ...
                            <tbody>
                                {loading ? (
                                    ...
                                    paginatedFatchData.map((product, index) => (
                                        <tr key={product.id} className="transition dark:hover:bg-slate-900 hover:bg-slate-50">
                                            ...
                                            <td className="border-b dark:border-sky-950 border-slate-200 px-4 py-3">
                                                <span
                                                    className={`inline-flex rounded-full px-2.5 py-1 text-xs font-medium ${isActive(product.status) ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-700'}`}
                                                >
                                                    {isActive(product.status) ? 'Active' : 'Inactive'}
                                                </span>
                                            </td>
                                            <td className="border-b dark:border-sky-950 border-slate-200 px-4 py-3">
